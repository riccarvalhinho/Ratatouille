...
Guardar as imagens em `docs/design/inspiracoes/` e referenciá-las aqui com uma nota do que se quer
levar de cada uma. Uma referência sem nota não serve: o que interessa não é a imagem, é o que dela se
vai aproveitar.

Exemplo do formato:

```
### Bimby — ecrã de execução
![](inspiracoes/bimby-execucao.jpg)
A levar: um passo de cada vez, ocupando o ecrã todo, com o timer integrado no passo em vez de num
canto. A evitar: a barra de navegação inferior, demasiado pequena para toque com mãos molhadas.
```
