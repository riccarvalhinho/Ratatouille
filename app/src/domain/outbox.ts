/**
 * Fila de escritas por enviar para o GitHub. Ver docs/adr/0004-escrita-via-github-api.md.
 *
 * Tudo aqui é puro: recebe a fila e devolve uma fila nova. Quem persiste e quem fala com a rede é
 * `app/src/data/outbox-store.ts`. Foi assim de propósito — a ADR avisa que é aqui que vão aparecer
 * os bugs difíceis, e um módulo sem I/O testa-se sem simular rede nenhuma.
 *
 * ## A unidade é o ficheiro, não a alteração
 *
 * Uma entrada da fila é "este ficheiro passa a ter este conteúdo", e a chave é o caminho. Planear e
 * desplanear a mesma refeição cinco vezes antes de haver rede deixa **uma** entrada, com o conteúdo
 * final — e portanto um commit, não cinco. É a resposta ao "a vigiar: o número de commits" da ADR.
 *
 * Funciona porque cada ficheiro é escrito por inteiro a partir do estado local: `data/planning` por
 * semana, `data/state/favourites.json`, `data/state/history.json`. Não há fusão parcial a fazer.
 *
 * ## Conflitos
 *
 * Escrever pede o `sha` da versão que se está a substituir. Se o ficheiro tiver mudado no GitHub
 * entretanto, a API recusa; a resposta é reler o `sha` e voltar a escrever — **a última escrita
 * ganha**. É o comportamento certo para um utilizador só (Q7), e é o compromisso que permite não ter
 * servidor. Se um dia houver duas pessoas a planear, isto tem de mudar.
 */

export interface OutboxEntry {
  /** Caminho no repositório, ex.: `data/planning/2026-W35.json`. É a chave da fila. */
  path: string;
  /** Conteúdo completo do ficheiro, já serializado. */
  content: string;
  /** Mensagem do commit. */
  message: string;
...
/**
 * Teto de cinco minutos.
 *
 * Sem teto, uma noite sem rede levava o intervalo a horas e a manhã seguinte começava a app com o
 * plano por enviar sem nada acontecer durante muito tempo. Cinco minutos é curto para não desistir
 * e longo para não martelar a API.
 */
 ...
 * enviado já não é o que está no ecrã. Sem isto, planear durante uma sincronização perdia-se.
 */
export function markSent(queue: OutboxEntry[], path: string, sentContent: string): OutboxEntry[] {
...
export const repoPaths = {
  week: (week: string) => `data/planning/${week}.json`,
  favourites: 'data/state/favourites.json',
  history: 'data/state/history.json',
} as const;
